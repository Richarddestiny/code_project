...
{ 
  ...
  { 
      setgpio_1k_low(); 
      gpio_set(CPU_GPIO2, (ndata & 0x0001)); 
    setgpio_1k_high(); 
    ndata >>= 1; 
  } 
  setgpio_1k_low(); 
...
int init_camera_i2c(void)
{
   ...
   gpio_set(CPU_GPIO3,1);
    ...
    pcie_gpio_fd = open(I2C_DEV,O_RDWR);
    if(pcie_gpio_fd < 0)
    {
