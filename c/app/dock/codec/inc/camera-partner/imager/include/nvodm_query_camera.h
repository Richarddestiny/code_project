...
typedef struct PCLDeviceSpecRec {
    ...
    NvU64 PowerOnSequence;
    NvU32 PowerOffSequenceNum;
    NvU64 PowerOffSequence;
    NvU32 ClockNum;
    NvU64 ClockNames;
    NvU64 AlterName;
    NvU32 DetectNum;
...
// number of different typs of devices PCLModule supported
// sensor/focuser/flash/rom, etc.
#define CAMERA_DEVICE_TYPES    4
/**
 * A Camera Module container.
 */
typedef struct CameraModuleRec {
    NvU32 ID;
    ...
    char Badge[CAMERA_MAX_NAME_LENGTH];
    NvU8 Position;
} NvOdmCameraModule;

typedef struct CameraModuleHeaderRec {
    NvU32 Version;
    NvU32 Size;
    NvU32 ModuleNum;
    NvU32 DeviceNum;
...
enum {
    CAMERA_QUERY_GUIDS,
    CAMERA_QUERY_DEVICES,
};
const void *NvOdmImagerQuery(NvU32 QueryType, NvU32 *Num);

// deprecated function. still keep it since this header file is included in some bootloader odm_query
// code and this function is called although the fucntion itself is implemented in bootloader.
// should be removed once above issue is solved.
