...
typedef enum {
    ...
    /* The following are implicitly 10 bits per pixel */
    ...
    /* The different Bayer CFA configurations with 8 bits per pixel */
    ...
    /* The different Bayer CFA configurations with 12 bits per pixel */
    NvOdmImagerPixelType_BayerRGGB12,
    NvOdmImagerPixelType_BayerBGGR12,
    NvOdmImagerPixelType_BayerGRBG12,
    NvOdmImagerPixelType_BayerGBRG12,
...
typedef struct {
    DeviceProfile *pDev;
    NvViCacheHandle hRegCache;
} NvOdmImagerDeviceCache;
...
typedef enum
{
    ...
    /// Specifies the driver register write functionality to use register
    /// cache where features support
    /// Expected Set value is a ::NvOdmImagerDeviceCache; NULL disables cache switch
    /// Expected Get value is a ::NvBool to signify driver support
    NvOdmImagerParameter_CacheEnable,
...
typedef struct NvOdmImagerStaticPropertyRec
{
    ...
    // Holds the amount of delay in frames it takes for
    // Group Hold settings update to appear at the output stream.
    // Settings will be written before a vblank, and the delay
    // starts counting from the vblank.
    // For core, it is used as a count of the number of bubble frames in
    // the system to account for the delay in applying exposure settings
    // in the sensor.
    // The value can be found under Grouped Parameter Hold timing in
    // sensor data sheets. Normally, settings will appear on the
    // 2nd frame after update, giving us a delay of 1 frame.
    // This can change depending on vendor specifications.
    NvU32 GroupHoldSyncDelayFromVblank;
