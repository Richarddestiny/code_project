...
// Currently we send 1 partial result for AE/AWB/AF hence the count is 1
// set the correct count as more clients are added.
#define SUPPORTED_PARTIAL_RESULT_COUNT (1)
...
// Bug 1539055: Remove NvCamColorCorrectionMode enum as it is not used,
// instead NvBool ColorCorrectionMatrixEnable is used.
/**
...
typedef enum
{
    ...
    /* Manual use case for direct application control of capture
...
typedef enum
{
    ...
    NvCamWbManualMode_ExternalGains,
    // TODO: Bug 1537371-add manual CCT control (and CCT/tint vectors)
} NvCamWbManualMode;
...
typedef struct NvCamProperty_Public_Controls
{
    ...
    // Custom property: Does not map to any android property
    // This flag decides whether the ColorCorrectionMatrix
    // will be used or not.
    NvBool ColorCorrectionMatrixEnable;

    // maps to android.colorCorrection.transform
    NvIspMathF32Matrix ColorCorrectionMatrix;

    // Pull these properties out as they are implemented.
    // ######################### NOT IMPLEMENTED ################################

    // Bug 1539055: Remove ColorCorrectionMode as it is not used,
    // instead ColorCorrectionMatrixEnable is used.
...
typedef struct NvCamProperty_Public_Static
{
    ...
    // List of max number of regions that can be listed for metering,
    // each for AE, AWB, AF respectively.
    NvCamPropertyList MaxRegions;
#else
    // maps to android.control.maxRegions
