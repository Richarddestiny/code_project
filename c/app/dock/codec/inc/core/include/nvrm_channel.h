...
/**
 * Creates duplicate of channel. It is similar as NvRmChannelOpen() in
 * functionality but It uses IOCTL instead of open() system call to get
 * channel handle.
 *
 * @param gChannel Handle to Channel whose duplicate is to be created.
 *     gChannel must not be NULL.
 * @param phChannel Pointer to be filled in with the new channel handle.
 */

NvError NvRmChannelDup(
    NvRmChannelHandle gChannel,
    NvRmChannelHandle * phChannel );
...
/**
 * Sets error notifier for a channel
 *
 * @param hChannel The channel handle to set error notifiers.
 */

NvError NvRmChannelSetErrNotifier(
    NvRmChannelHandle phChannel );
...
/**
 * Obtains the index of a dynamically allocated host managed sync point register
 * for a particular <channel, module> pair.
 *
 ...
 * @param pSyncPointID The sync point, if a free sync point is found for the
...
/**
 * Obtains the index of a dynamically allocated client managed sync point register for a
 * particular module.
 *
 * It is not allowed to set priority on a channel or stream which has (or will
 * have) retrieved syncpoint IDs via this call.
 *
 * @param name The name to be given to syncpt for debugging purpose
 * @param SyncPointIndex Index for the module.
 * @param pSyncPointID The sync point, if a free sync point is found for the
 *        given module, otherwise an error is returned.
 */

 NvError NvRmChannelGetClientManagedSyncPoint(
    NvRmChannelHandle hChannel,
    const char *name,
    NvU32 SyncPointIndex,
    NvU32 *pSyncPointID );
...
/**
 * Frees the dynamically allocated syncpt id.
 *
 * @param SyncPointIndex Index for the module.
 * @param pSyncPointID The sync point id, which needs to be freed.
 */

 void NvRmChannelFreeClientManagedSyncPoint(
    NvRmChannelHandle hChannel,
    NvU32 SyncPointIndex,
    NvU32 pSyncPointID );
...
/**
 * Checks if setclass is a protected command that should not be used inside
 * command buffer
 */
  NvBool NvRmChannelGatherFilterEnabled(void);
...
/**
 * Add a host wait for the fence fd before the stream. Caller retains
 * ownership of the fence fd. Fence fd has to be kept open until stream has
 * been flushed.
 *
 * There is one pre-fence per stream. Setting a new one will overwrite the old
 * one.
 *
 * @param pStream The stream
 * @param fence_fd The fence to wait for
 */
void NvRmStreamSetFenceFd(
    NvRmStream *pStream,
    int fence_fd);
...
/**
 * Same as NvRmStreamFlush, but returns a sync fd.
 *
 * @param pStream The stream to flush.
 * @return fence fd for synchronization
 */
int NvRmStreamFlushFenceFd(NvRmStream *pStream, ...);
